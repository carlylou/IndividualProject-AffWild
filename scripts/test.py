...
# def compute_loss(sess, prediction, labels_batch):
#     print 'enter loss'
#     mse_total = []
#     ccc_total = []
...
#
#         test_pred_single = tf.reshape(predictions[:, i], [-1])
#         test_gt_single = tf.reshape(labels[:, i], [-1])
#
#         v1, v2, v3, v4 = sess.run([pred_single, gt_single, test_pred_single, test_gt_single])
#         print np.shape(v1), np.shape(v2), np.shape(v3), np.shape(v4)
#         # compute ccc
...
def get_restore_variable(data_path):
